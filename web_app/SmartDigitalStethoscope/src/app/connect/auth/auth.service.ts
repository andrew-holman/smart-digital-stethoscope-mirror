...
export class AuthService {
  ...
  public async userId(): Promise<string> {
    return this.authenticated ? (await this.user).uid : "";
  }
  ...
  public registerNew(
    ...
    password: string,
    displayName: string = ""
  ): Promise<[User, auth.GoogleAuthProvider]> {
        ...
        .then((credential) =>
          credential.user
            ...
            .then(() => {
              ...
              return [credential.user, provider];
            })
        )
    );
  }
   ...
   * @returns the authenticated User object
   */
  public async refresh(password: string): Promise<User> {
    console.log("Refreshing authentication: ", (await this.user).email);
    // Gets fresh credentials for the current user
    const credential = auth.EmailAuthProvider.credential(
      (await this.user).email,
      password
    );
